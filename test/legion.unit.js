...
describe('Legion unit test', async () => {
    ...
    it('should not RequestJoin because of pasportFee less then required', async () => {
        ...
        await expect(
            legionMock.connect(user).RequestJoin(userTgId, codeName, parentName, publicKey, {
                value: fValue
            })
    ...
    it('should RequestJoin', async () => {
        ...
        await expect(requestJoin)
            ...
            .withArgs(userTgId, user.address, await legionMock.GetWalletByNickName(parentName))
    ...
    it('should NOT accept join because only parent_name can accept it', async () => {
        await expect(legionMock.connect(user).AcceptJoin(userTgId, codeName)).to.be.revertedWith(
            'only parent_name can accept it'
        )
    })
    ...
    it('should delete user', async () => {
        await legionMock.connect(user).RequestJoin(userTgId, codeName, parentName, publicKey, {
            value: pasportFee
        })
    ...
    it('should AcceptJoin', async () => {
        ...
        const acceptJoin = await legionMock.connect(admin).AcceptJoin(userTgId, parentName)
    ...
    it('should NOT DeclineRequest because already approved OR do not exists yet', async () => {
        ...
        await expect(legionMock.connect(owner).DeclineRequest(userTgId)).to.be.revertedWith(
    ...
    it('should DeclineRequest', async () => {
        ...
        const declineRequest = await legionMock.connect(user).DeclineRequest(userTgId)
    ...
    it('should NOT ClearParent because users allowed only to clear themselfs', async () => {
        await expect(legionMock.connect(user).ClearParent(parentName, codeName)).to.be.revertedWith(
    ...
    it('shoukd NOT devDeleteUser because has no moderator role', async () => {
        await expect(legionMock.connect(turing).devDeleteUser(user.address)).to.be.revertedWith(
    ...
    it("should NOT devDeleteUser because this function work's only for testnet", async () => {
        await expect(legionMock.connect(owner).devDeleteUser(user.address)).to.be.revertedWith(
            "this function work's only for testnet"
        )
    })

    it('should devInitAdmin', async () => {
        await expect(legionMock.connect(newAdmin).devInitAdmin(newAdmin.address, 777, publicKey))
    ...
    it('should get user by tg id', async () => {
        const userByTgId = await legionMock.connect(user).GetUserByTgId(constructortTgId)
    ...
    it('should get public key by address', async () => {
        const getPublicKey = await legionMock.connect(user).GetPublicKeyByAddress(admin.address)
        expect(getPublicKey).to.equal(publicKey)
    })

    it('should get user by nickname', async () => {
        const getUserByNickname = await legionMock.connect(user).GetUserByNickName(parentName)
    ...
    it('should get wallet by nickname', async () => {
        const getWallet = await legionMock.connect(user).GetWalletByNickName(parentName)
        expect(getWallet).to.equal(admin.address)
    })

    it('should get user by address', async () => {
        const getUser = await legionMock.connect(user).GetUserByAddress(admin.address)
    ...
    it('should get tg id by address', async () => {
        const tgIdByAddress = await legionMock.connect(user).GetTgIdByAddress(admin.address)
        expect(tgIdByAddress).to.equal(constructortTgId)
    })

    it('should get user wallet by tg id', async () => {
        const tgIdByAddress = await legionMock.connect(user).GetUserWalletByID(constructortTgId)
