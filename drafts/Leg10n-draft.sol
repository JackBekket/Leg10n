...
contract Leg10n is Ownable, AccessControl {
   ...
   struct User {
      address userAddress;
      string tgId;      // unic Id for telegram (number) plain text is int64, but we store encrypted value
      bool valid;
   ...
   event joinRequested(string applyerTg, address wallet_address, address indexed parent_address);
   event joinRequestedIndexedTG(string applyerTg, address wallet_address, address indexed parent_address);
   event requestAccepted(string indexed applyerTg, address user_address, address parent_address);
   ...
   constructor(address turing_,address admin_,string memory tgid_, string memory public_key_) Ownable() {
      ...
      if (admin_ == address(0x0)) {
         ...
         _devInitAdmin(msg.sender,tgid_,public_key_);
      } else {
   ...
   function _updateAddress(string memory tgId, address userAddress, string memory code_name_,string memory parent_name) internal {
    ...
    */
   function RequestJoin(string memory applyerTg, string memory code_name_, string memory parent_name, string memory public_key) public payable {
   ...
   */
   ...
   function _devInitAdmin(address admin_, string memory tgid_, string memory public_key_) internal  {
   ...
   function GetTgIdByAddress(address user_wallet) public view returns(string memory tgid) {
